...
export const BACKEND_TABLES: TableMeta[] = [
  ...
  },
  {
    ...
    columns: [
      ...
      { name: 'Status', type: 'enum', note: 'legacy To Do | In Progress | Done (StatusId wins)' },
      { name: 'ItTicket', type: 'string?' },
      { name: 'Number', type: 'int?', note: 'sequential within the project (KEY-12)' },
      { name: 'StatusId', type: 'int?', fk: 'ProjStatuses.Id' },
      { name: 'Description', type: 'string?' },
      { name: 'Priority', type: 'enum?', note: 'LOW | MEDIUM | HIGH | URGENT' },
      { name: 'ParentId', type: 'int?', note: 'subtask parent, one level only' },
      { name: 'StartDate', type: 'date?' },
      { name: 'DueDate', type: 'date?' },
      { name: 'SortOrder', type: 'float?', note: 'position in the Kanban column' },
  ...
  },
  {
    ...
    entity: 'Contact',
    kind: 'list',
    description: 'Contact directory outside the team, searchable by topic (with a Library procedure pointer).',
    columns: [
      { name: 'Id', type: 'int', pk: true },
      { name: 'Name', type: 'string' },
      { name: 'Role', type: 'string?' },
      { name: 'Department', type: 'string?' },
      { name: 'Company', type: 'string?', note: 'when external (auditor, custodian…)' },
      { name: 'Email', type: 'string?' },
      { name: 'Phone', type: 'string?' },
      { name: 'Topics', type: 'string?', note: 'comma-separated searchable keywords' },
      { name: 'Notes', type: 'string?' },
      { name: 'Procedure', type: 'string?', note: 'Library search term for the related procedure' },
    ],
    endpoints: [],
  },
  {
    ...
    entity: 'ProjStatus',
    kind: 'list',
    description: 'Kanban columns, per project (Projects module).',
    columns: [
      { name: 'Id', type: 'int', pk: true },
      { name: 'ProjectId', type: 'int', fk: 'Projects.Id' },
      { name: 'Name', type: 'string' },
      ...
      { name: 'IsDone', type: 'bool?', note: 'tasks in this column count as done' },
    ],
    endpoints: [],
  },
  {
    ...
    entity: 'ProjComment',
    kind: 'list',
    description: 'Task comments, signed by the Windows user.',
    columns: [
      { name: 'Id', type: 'int', pk: true },
      { name: 'TaskId', type: 'int', fk: 'ProjectTasks.Id' },
  ...
  {
    key: 'projActivities',
    table: 'ProjActivities',
    entity: 'ProjActivity',
    kind: 'list',
    description: 'Task activity log: who changed what, and when.',
    columns: [
      { name: 'Id', type: 'int', pk: true },
      { name: 'TaskId', type: 'int', fk: 'ProjectTasks.Id' },
      { name: 'Actor', type: 'string?' },
      { name: 'Type', type: 'string', note: 'created | status | assignee | priority | dates | title | description | comment | subtask | detached' },
  ...
  },
  {
    ...
    entity: 'FinStatement',
    kind: 'list',
    description: 'Financial statements per entity+date+kind (balanceSheet | pnl | equity) with line items.',
    columns: [
      { name: 'Id', type: 'bigint', pk: true },
      { name: 'Entity', type: 'string' },
      { name: 'Date', type: 'date' },
    ...
    endpoints: [
      { method: 'GET', path: '/api/fin-statements?entity=&date=&kind=', desc: 'List (filterable)' },
      { method: 'PUT', path: '/api/fin-statements/{entity}/{date}/{kind}', desc: 'Upsert the full statement' },
      { method: 'DELETE', path: '/api/fin-statements/{entity}/{date}/{kind}', desc: 'Delete' },
    ],
  },
  {
    key: 'scenarios',
    ...
    entity: 'Scenario',
    kind: 'list',
    description: 'What-if scenarios (shocks on CET1/RWA, HQLA/flows, ASF/RSF) per entity+baseline date.',
    columns: [
      { name: 'Id', type: 'bigint', pk: true },
      { name: 'Entity', type: 'string' },
      ...
      { name: 'Name', type: 'string' },
      { name: 'Description', type: 'string?' },
      { name: 'Shocks', type: 'ScenarioShock[]', fk: 'ScenarioShocks.ScenarioId', note: 'target (capital|lcr|nsfr) · code · label · amount' },
    ],
    endpoints: [
      { method: 'GET', path: '/api/scenarios?entity=', desc: 'List' },
      { method: 'POST', path: '/api/scenarios', desc: 'Create' },
      { method: 'PUT', path: '/api/scenarios/{id}', desc: 'Replace (with shocks)' },
      { method: 'DELETE', path: '/api/scenarios/{id}', desc: 'Delete' },
    ],
  },
  {
    key: 'prodMappingEntries',
    table: 'ProdMappingEntries',
    entity: 'ProdMappingEntry',
    kind: 'list',
    description: 'Adjustments mapping workbook (Mapping.xlsb), stored relationally: gl (LIGNE → GL account + cp_TypeOf/SubType) · fx (CCY → CHF rate) · rt01 (CATEG → QDL) · industry (IND code, interco flags) · label (prefix labels) · hfm / hfmlabel / hfmrule (IFRS-HFM mapping). Editable here in place (audited); bulk update = re-upload the workbook in Production → Adjustments and 💾 save.',
    columns: [
      { name: 'Id', type: 'bigint', pk: true },
      { name: 'Kind', type: 'string', note: 'gl | fx | rt01 | industry | label | hfm | hfmlabel | hfmrule' },
      { name: 'MapKey', type: 'string', note: 'LIGNE / CCY / CATEG / IND / prefix / account' },
      { name: 'TextValue', type: 'string?' },
      { name: 'NumValue', type: 'float?', note: 'fx: CHF rate per 1 unit' },
      { name: 'TypeOf', type: 'string?' },
      { name: 'SubType', type: 'string?' },
      { name: 'EconomicActivityType', type: 'string?' },
      { name: 'Interco', type: 'string?', note: 'HYPERIOD_INTERCO group company' },
      { name: 'Description', type: 'string?' },
    ],
    endpoints: [
      { method: 'GET', path: '/api/data', desc: 'Part of the central document' },
    ],
  },
  {
    key: 'prodBaselines',
    ...
    entity: 'ProdBaseline',
    kind: 'list',
    description: 'Certified production baselines: the periods declared correct (who, when, which loads) — the drift controls compare against the latest one.',
    columns: [
      { name: 'Id', type: 'bigint', pk: true },
      { name: 'Entity', type: 'string' },
      { name: 'Date', type: 'date' },
      { name: 'LoadIds', type: 'string?' },
      { name: 'CollectionId', type: 'string?' },
      { name: 'CertifiedBy', type: 'string' },
      { name: 'CertifiedAt', type: 'datetime' },
